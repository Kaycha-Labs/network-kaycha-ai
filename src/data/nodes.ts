...
export const NODES: Record<string, NodeData> = {
  ironman: {
    ...
    services: [
      { name: 'LLM Gateway Proxy', port: '4000', detail: 'All fleet LLM calls routed here — audit + cost + routing + AP Memory dual-write (NSSM: LLMGateway)' },
  ...
  ironpatriot: {
    ...
    apps: [
      ...
      { name: 'jarvis-memory-mcp', category: 'ai', detail: 'QUBO memory optimizer SSE server — 9-term BQM, 3-way retrieval', port: '7472' },
      { name: 'AP Memory', category: 'ai', detail: 'HRR lattice memory — PRIMARY instance (NSSM: APMemory)', port: '8105' },
      { name: 'jarvis-rag', category: 'ai', detail: 'RAG API — 1.4M chunks, Qdrant + BM25 hybrid search', port: '8100' },
    ...
    services: [
      ...
      { name: 'jarvis-memory-mcp', port: '7472', detail: 'QUBO memory optimizer — 9-term BQM, 3-way retrieval (memory + RAG + AP Memory)' },
      { name: 'AP Memory (Primary)', port: '8105', detail: 'HRR lattice — SpaCy atomizer + pgvector HNSW + braid topology, syncs → AI server backup' },
      { name: 'jarvis-rag', port: '8100', detail: 'RAG API — 1.4M chunks, Qdrant vector + BM25 keyword hybrid search' },
      { name: 'jarvis-rag MCP', port: '8101', detail: 'MCP SSE proxy for rag_search, rag_context, rag_collections' },
  ...
  jarvis: {
    ...
    apps: [
      ...
      { name: 'jarvis-rag', category: 'ai', detail: 'RAG server — 1.4M chunks, 7 collections, Qdrant + BM25 hybrid', port: '8100' },
      { name: 'jarvis-memory', category: 'ai', detail: 'Memory optimizer Docker container (backup)', port: '7472' },
      { name: 'AP Memory', category: 'ai', detail: 'HRR lattice — BACKUP instance (Docker), synced from Iron-Patriot', port: '8105' },
    ...
    services: [
      ...
      { name: 'jarvis-rag', port: '8100', detail: 'RAG server (Docker) — 1.4M chunks across 7 collections, Qdrant + BM25 hybrid' },
      { name: 'jarvis-memory', port: '7472', detail: 'Memory optimizer (Docker) — backup instance' },
      { name: 'AP Memory (Backup)', port: '8105', detail: 'HRR lattice backup (Docker) — synced from Iron-Patriot primary' },
